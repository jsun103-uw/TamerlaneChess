...
The client and server both contain a Game instnace. The client never initiates any changes to its own Game instance by itself. The server is the source of truth, and the client sends move requests to the server in order to make a move, and the server will notify the client when a move has been successfully executed by them or their opponent. Only then will the client execute the same move, to synchronize the client with the server.
